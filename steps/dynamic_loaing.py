...
from pageobjects.dyemic_loading import DynamicLoadingPageObject


@given('the user open dynamic loading page')
def step_impl(context):
    context.current_page = DynamicLoadingPageObject()
...
def step_impl(context):
    context.current_page.click_element()
    time.sleep(3)


@when('the user click start button')
def step_impl(context):
    context.current_page.click_start()
    time.sleep(3)
